...
\begin{figure*}
  \begin{minipage}{3in}
    \includegraphics[width=3in]{install_page_big.eps}
    \centerline{\small{(a)}}
  \end{minipage}
  \begin{minipage}{2in}
    \vspace{0.47in}
    \includegraphics[width=2in]{add_server_big.eps}
    \centerline{\small{(b)}}
  \end{minipage}
  \begin{minipage}{2in}
    \vspace{0.47in}
    \includegraphics[width=2in]{processor_assignment_big.eps}
    \centerline{\small{(c)}}
  \end{minipage}

  \begin{minipage}{3in}
    \includegraphics[width=3in]{cluster_list_big.eps}
    \centerline{\small{(d)}}
  \end{minipage}
  \begin{minipage}{1in}
    \hspace{1in}
  \end{minipage}
  \begin{minipage}{3in}
    \includegraphics[width=3in]{classifier_big.eps}
    \centerline{\small{(e)}}
  \end{minipage}
...
(Supplementary Materials, Section~D.1), generating the fragments from
...
Materials).  \mc{The most important method of quality assessment is
  ...
  of true positives, false positives, and true negatives), {\it Type 1
  ...
  materials.  In Figure~\ref{SeJiT1T2} we plot these four tests as a
  function of error rate, and observe the almost identical results
  between \peace\/ and \wcd\/.  In Figure~\ref{seq_parallel} we plot
  the runtime for \peace\/ and \wcd\/, again observing almost identical
  results when run sequentially -- but significantly faster runtime for
  PEACE on multiple processor when holding the EST/processor ratio
  constant (ranging from a 65\% improvement for two processors too a
  17\% improvement for 12 processors).}
...
\cite{Hazelhurst08a,Hazelhurst08b}, with results in
Table~\ref{SangerReal}.  We notice essentially identical
...
with PEACE showing a 60\% runtime improvement in the smaller data set,
requiring 20\% more time in the Arabidopsis data set.  In Section~D.1
...
\mc{We tested the three tools on short-read data using the MetaSim
  ...
  Leaving \capthree\/ and \metasim\/ at their default values, but
  ...
  numbers.  For 100 transcript \peace creates 128 clusters (as opposed
  to 6,370 and 19245 clusters for \wcd and \capthree), and creates
  only 3 singletons (as opposed to 5,850 and 15432).  We find it
  likely that the \wcd results could be considerably improved by the
  simple adjustment of a few parameters, but were unable to determine
  how to do that.  (Runtime of \peace\/ was somewhat slower than that
  of \wcd, but given the significant improvement in quality results we
  do not find this to be very troubling.)}
...
% LocalWords:  NGS polyA set's GHz Xeon Bikun Arabidopsis JVM Jaccard LV
