...
\begin{abstract}
  ...
  process large data sets of transcripts fragments of length 50 bases
  ...
  competing clustering tools.  Once clusered, the user can use the GUI
  to to collect statistics and single out specific clusters for more
  comprehensive study.  \peace\/, when used as a ``pre-assembly''
  tool, enables any assembly tool to work with individual small
  clusters of closely related sequences.  The result is a significant
  increase in sensitivity, as well as improved robustness to
  sequencing error as well as a reduction in runtime.  For a 5\%
  sequencing error rate, we see a 52\% improvement in sensitivity over
  the leading cluster tool, \wcd\/, a 184\% improvement in sensitivity
  over the assembly tool \capthree.  When applied to a 361 Mb set of
