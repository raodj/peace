...
\begin{abstract}
  ...
  competing clustering tools.  Once clustered, the user can use the GUI
  to collect statistics and single out specific clusters for more
  ...
  increase in sensitivity, improved robustness to sequencing error,
  and a reduction in runtime.  For a 5\% sequencing error rate, we see
  a 52\% improvement in sensitivity over the leading cluster tool,
  \wcd\/, and 184\% improvement in sensitivity over the assembly tool
  \capthree.  When applied to a 361 Mb set of Arabidopsis transcriptome
  fragments, we see a 40\% speedup over \wcd.
\end{abstract}
