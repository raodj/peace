...
Materials).  \mc{The most important method of quality assessment is
  ...
  function of erro rate, and observe the almost identical results
  between \peace\/ and \wcd\/.  In Figure~\ref{seq_parallel} we plot
  the runtime for \peace\/ and \wcd\/, again observing almost identical
  results when run sequentially -- but signifcantly faster runtime for
  PEACE on multiple processer when holding the EST/processor ratio
...
Benchmark Dataset used to test \easycluster\/ \cite{Picardi09}, and
the A076941.fa {\it Arabidopsis thaliana} dataset used to test WCD
\cite{Hazelhurst08,Hazelhurst08a}, with results in
...
with PEACE showing a 60\% runtime improvement in the smaller dataset,
requiring 20\% more time in the Arabidopsis dataset.  In Section~D.1
...
\mc{We tested the three tools on short-read data using the MetaSim tool of
{\it Richter et al.} [\cite{Richter2008}].  Encoded into MetaSim are
modules configured to emulate the outpspecific tecnology,
including the 454 and Illumina sequences.  For 454, the simulator  }
...
sequential mode on a .  When applied to a 64 Mb set of
