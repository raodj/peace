...
/** \typedef StringIntMap ColorCodeMap;
...
*/
typedef StringIntMap ColorCodeMap;

/** \typedef HashMap<int, bool> ClusterUseList;

    \brief A short cut typedef to conveniently use a hash_map.

    The ClusterUseList is used to rapidly look up if color codes must
    be assigned for a given cluster ID. This hash map is optional and
    is passed in as a parameter by derived tool classes for used by
    the ShowAlignment::loadClusterInfo() method.
*/
typedef HashMap<int, bool> ClusterUseList;
...
public:
    ...
    */
    ...
    /** Parse comma separated list of cluster indexes into hash map.

        This method is used from the loadClusterInfo() method to parse
        an optional list of comma separated cluster indexes to be
        colored by this method. This method parses the string into a
        hash map to facilitate rapid look up during cluster
        processing.

        \param[in] clusterList A string containing a comma separated
        list of cluster index values.
...
protected:
    ...
    */
	...
	/** Hash map to look up the subset of clusters to be colored.

		This has map contains the subset of clusters that must
		actually be colored (while rest of the clusters are rendered
		in black). This instance variable is populated via a call to
		the setClustersToColor() method. It is used in the getColor()
		method.
	*/
