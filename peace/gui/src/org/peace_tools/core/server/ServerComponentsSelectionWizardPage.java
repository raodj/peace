...
/**
 * This class serves as one of the (namely 3rd page) interactive pages in the
 * ServerWizard. This page permits the user to select the software components to
 * be installed on the remote server. The software components available as
 * choices are:
 * 
 * <ul>
 * <li>Fresh install of PEACE (installed from the tar ball in the GUI).
 * Optionally (later on) the user will be permitted to reuse an existing install
 * of PEACE or a SVN version of PEACE.</li>
 * 
 * <li>Install the EAST assembler from the tar ball (or use an existing copy of
 * EAST, similar to PEACE). This available only on Linux boxes right now.</li>
 * 
 * <li>An install of DECAGON (Distributed Environment for Comparative Analysis
 * of Genomic-Assemblers) for comparing various assemblers. This is available on
 * Linux machines.</li>
 * </ul>
 */
public class ServerComponentsSelectionWizardPage extends GenericWizardPage implements ItemListener {
	 ...
	 *               about the server.
	 */
	public ServerComponentsSelectionWizardPage(ServerWizard wizard) {
		this.wizard = wizard;
		...
		// Create a panel with the type combo box for the user to choose from two
		// different options.
		...
		// Create a sub panel with choices for the source of software components to
		// install on the server.
		softCompSelections = new boolean[] { true, false, false };
		guiSoftComps = new JPanel[3];
		// Create component for PEACE selection.
		guiSoftComps[0] = createCompSelPanel(0, "NewCluster", softCompSelections[0], true, false);
		// Create component for EAST selection.
		guiSoftComps[1] = createCompSelPanel(1, "NewMST", softCompSelections[1], false, true);
		// Create component for DECAGON selection.
		guiSoftComps[2] = createCompSelPanel(2, "Jobs", softCompSelections[2], false, false);

		// Pack the various components into a suitable panel.
		JPanel choicePanel = Utilities.createLabeledComponents(null, null, 0, true, guiSoftComps[0],
				Box.createVerticalStrut(10), guiSoftComps[1], Box.createVerticalStrut(10), guiSoftComps[2]);
		add(choicePanel, BorderLayout.CENTER);
	}

	private JPanel createCompSelPanel(int choiceIndex, String iconName, boolean checked, boolean enabled,
			boolean editable) {
		...
		if (editable) {
			...
			JCheckBox cb = new JCheckBox(SOFT_COMP_LABELS[choiceIndex * 2], checked);
			...
			// Not editable entry. Create a JLabel with a suitable checkbox-like icon.
			String cbName = "images/16x16/" + (checked ? "Checked" : "") + "Box.png";
			selector = new JLabel(SOFT_COMP_LABELS[choiceIndex * 2], Utilities.getIcon(cbName), SwingConstants.LEFT);
		}

		// Make the selector font bold & prominent.
		Utilities.adjustFont(selector, 0, 0, 1);
		...
		JPanel infoPanel = createInfoPanel(SOFT_COMP_LABELS[choiceIndex * 2 + 1], enabled);
	...
	private JPanel createInfoPanel(String infoMsg, boolean enabled) {
		// Break the information message into individual lines.
	...
	public boolean pageChanging(WizardDialog dialog, int currPage, int nextPage) {
		...
		wizard.getServer().setEASTInstalled(softCompSelections[1]);
		return true;
	}
	...
	/**
	 * The combo-box that permits the user to choose the source for software
	 * components --- between components from local install package or remote
	 * SVN/existing copy.
	 */
	private final JComboBox<?> softCompSrc;

	/**
	 * An array of check-boxes that permits the user to choose the sub-set of PEACE
	 * server software components to be installed on the server. The values in this
	 * array correspond to:
	 * 
	 * <ol>
	 ...
	 */
	private final boolean[] softCompSelections;

	/**
	 * The list of GUI-components that contain a label and check box for the
	 * software components being installed. These GUI components are used to merely
	 * enable/disable labels to provide the user with better visual feedback.
	 */
	private final JPanel[] guiSoftComps;

	/**
	 * An array of static labels that are used to provide the users with information
	 * about the software components that are to be installed on the server.
	 */
	private static final String[] SOFT_COMP_LABELS = { "PEACE Clustering Engine",
