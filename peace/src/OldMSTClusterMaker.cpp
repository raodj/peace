#ifndef OLD_MST_CLUSTER_MAKER_CPP
#define OLD_MST_CLUSTER_MAKER_CPP
...
extern char DefCacheType[10];

// The static instance variables for command line arguments.
int    OldMSTClusterMaker::cacheSize     = 128;
bool   OldMSTClusterMaker::noCacheRepop  = true;
bool   OldMSTClusterMaker::strictOrder   = false;
char*  OldMSTClusterMaker::inputMSTFile  = NULL;
char*  OldMSTClusterMaker::outputMSTFile = NULL;
bool   OldMSTClusterMaker::dontCluster   = false;
bool   OldMSTClusterMaker::prettyPrint   = false;
bool   OldMSTClusterMaker::guiPrint      = false;
int    OldMSTClusterMaker::maxUse        = -1;
char*  OldMSTClusterMaker::cacheType     = DefCacheType;
float  OldMSTClusterMaker::clsThreshold  = 130;
char*  OldMSTClusterMaker::progFileName  = NULL;
    
// The common set of arguments for all FW EST analyzers
arg_parser::arg_record OldMSTClusterMaker::argsList[] = {
    {"--cache", "#similarity metrics to cache per EST",
     &OldMSTClusterMaker::cacheSize, arg_parser::INTEGER},
    {"--no-cache-repop", "Suppress EST cache repopulation",
     &OldMSTClusterMaker::noCacheRepop, arg_parser::BOOLEAN},    
    {"--no-order", "Disable strict order of processing messages",
     &OldMSTClusterMaker::strictOrder, arg_parser::BOOLEAN},
    {"--input-mst-file", "Read MST data from file (skip parallel MST building)",
     &OldMSTClusterMaker::inputMSTFile, arg_parser::STRING},
    {"--output-mst-file", "Output MST data to file",
     &OldMSTClusterMaker::outputMSTFile, arg_parser::STRING},
    {"--dont-cluster", "Just generate MST data. Don't do clustering",
     &OldMSTClusterMaker::dontCluster, arg_parser::BOOLEAN},
    {"--pretty-print", "Print a pretty cluster tree.",
     &OldMSTClusterMaker::prettyPrint, arg_parser::BOOLEAN},
    {"--gui-print", "Print the cluster tree for GUI processing.",
     &OldMSTClusterMaker::guiPrint, arg_parser::BOOLEAN},
    {"--maxUse", "Set a threshold to aggressively use metrics (default=0)",
     &OldMSTClusterMaker::maxUse, arg_parser::INTEGER},
    {"--clsThreshold", "Set a threshold for clustering (default=1.0)",
     &OldMSTClusterMaker::clsThreshold, arg_parser::FLOAT},    
    {"--cacheType", "Set type of cache (heap or mlist) to use (default=heap)",
     &OldMSTClusterMaker::cacheType, arg_parser::STRING},
    {"--progress", "Log MST construction progress in a file (used by GUI)",
     &OldMSTClusterMaker::progFileName, arg_parser::STRING},        
    {NULL, NULL, NULL, arg_parser::BOOLEAN}
};

OldMSTClusterMaker::OldMSTClusterMaker(ESTAnalyzer *analyzer,
                                 const int refESTidx,
...
OldMSTClusterMaker::~OldMSTClusterMaker() {
    if (mst != NULL) {
        delete mst;
    }
}
...
void
OldMSTClusterMaker::showArguments(std::ostream& os) {
    ClusterMaker::showArguments(os);
    // Use a arg parser object to conveniently display common options.
    os << "Options for " << name << " are:\n";
    arg_parser ap(OldMSTClusterMaker::argsList);
    os << ap;
}

bool
OldMSTClusterMaker::parseArguments(int& argc, char **argv) {
    arg_parser ap(OldMSTClusterMaker::argsList);
...
void
OldMSTClusterMaker::estAdded(const int estIdx, std::vector<int>& repopulateList) {
...
int
OldMSTClusterMaker::managerUpdateCaches(int estIdx, const bool refreshEST) {
...
void
OldMSTClusterMaker::computeNextESTidx(int& parentESTidx, int& estToAdd,
...
void
OldMSTClusterMaker::addMoreChildESTs(const int parentESTidx, int& estToAdd,
    ...
    do {
        ...
        ASSERT ( estToAdd != -1 );
        mst->addNode(parentESTidx, estToAdd, metric, alignmentData, 0);
        if (--pendingESTs == 0) {
...
void
OldMSTClusterMaker::updateProgress(const int estsAnalyzed,
...
OldMSTClusterMaker::manager() {
    ...
    do {
        ...
        if ((maxUse == -1) || (parentESTidx == -1)) {
            ASSERT ( estToAdd != -1 );
            mst->addNode(parentESTidx, estToAdd, metric, alignmentInfo, 0);
            if (--pendingESTs == 0) {
                // All EST's have been added to the MST.  Nothing more to
                // do.  So break out of the while loop.
                break;
...
int
OldMSTClusterMaker::getOwnerProcess(const int estIdx) const {
...
float
OldMSTClusterMaker::analyze(const int otherEST) {
    return analyzer->analyze(otherEST);
}

int
OldMSTClusterMaker::initialize() {
...
void
OldMSTClusterMaker::populateCache(const int estIdx, SMList* metricList) {
...
void
OldMSTClusterMaker::getOwnedESTidx(int& startIndex, int& endIndex) {
    const int ESTsPerProcess = EST::getESTList().size() / MPI_GET_SIZE();
    const int ExtraESTs      = EST::getESTList().size() % MPI_GET_SIZE();
    const int MyRank         = MPI_GET_RANK();
...
void
OldMSTClusterMaker::displayStats(std::ostream& os) {
...
int
OldMSTClusterMaker::populateMST() {
...
int
OldMSTClusterMaker::addDummyCluster(const std::string name) {
...
void
OldMSTClusterMaker::addEST(const int clusterID, const int estIdx) {
...
int
OldMSTClusterMaker::buildAndShowClusters() {
...
int
OldMSTClusterMaker::makeClusters() {
    int result = NO_ERROR;
...
void
OldMSTClusterMaker::sendToWorkers(int data, const int tag) const {
    const int ProcessCount = MPI_GET_SIZE();
    for(int rank = 1; (rank < ProcessCount); rank++) {
...
bool
OldMSTClusterMaker::hasValidSMEntry(const SMList& list) const {
    if (list.empty()) {
