...
TwoPassD2::TwoPassD2() : FWAnalyzer("twopassD2") {
    s1WordTable     = NULL;
    s2WordTable     = NULL;
    delta           = NULL;
    alignmentMetric = 0;
    ...
    // sufficiently dissimilar that symmetric bounded D2 does not need
    // to be run.
    maxThreshold    = 130;
}
...
void
TwoPassD2::addCommandLineArguments(ArgParser& argParser) {
...
int
TwoPassD2::setReferenceEST(const EST* est) {
...
float
TwoPassD2::getMetric(const EST* otherEST) {
...
float
TwoPassD2::runD2Asymmetric(const EST* otherEST, int* s1MinScoreIdx, 
                           int* s2MinScoreIdx) {
...
float
TwoPassD2::runD2Bounded(const EST* otherEST, int sq1Start, int sq1End, 
