#ifndef SIMULATION_D2_CPP
#define SIMULATION_D2_CPP
...
char SimulationD2::CharToInt[256];

// Skip parameter for d2 asymmetric.  Defaults to 1 (i.e. d2 symmetric).
int SimulationD2::frameShift = 1;

// The number of comparisons to be done.
int SimulationD2::numComparisons = 10000;

static int MapSize;
static int BitMask;
static int NumWordsWin;

static int* fdHashMap;
static int* s1WordTable;
static int* s2WordTable;

// The set of arguments for this class.
arg_parser::arg_record SimulationD2::argsList[] = {
    {"--frameShift", "Frame Shift (default=1)",
     &SimulationD2::frameShift, arg_parser::INTEGER},
    {"--numComparisons", "Number of Comparisons (default=1000)",
     &SimulationD2::numComparisons, arg_parser::INTEGER},
    {NULL, NULL}
};

SimulationD2::SimulationD2(const int refESTidx,
                           const std::string& outputFileName)
    : FWAnalyzer("D2sim", refESTidx, outputFileName) {
...
void
SimulationD2::showArguments(std::ostream& os) {
    FWAnalyzer::showArguments(os);
    // Use a arg parser object to conveniently display common options.
    arg_parser ap(SimulationD2::argsList);
    os << ap;
}

bool
SimulationD2::parseArguments(int& argc, char **argv) {
    arg_parser ap(SimulationD2::argsList);
...
int
SimulationD2::setReferenceEST(const int estIdx) {
...
void
SimulationD2::buildWordTable(std::string s2) {
    // init ref-est word table
...
void
SimulationD2::buildFdHashMaps(int* sed) {
...
void
SimulationD2::refShiftUpdateFd(int* sed, const int framePos) {
...
void
SimulationD2::rightShiftUpdateFd(int* sed, const int framePos) {
    // update sed and fd from leftmost word falling out
...
void
SimulationD2::leftShiftUpdateFd(int* sed, const int framePos) {
...
std::string
SimulationD2::reverseComplement(std::string sequence) {
...
SimulationD2::analyze() {
    ...
    for (int i = 0; i < numComparisons; i++) {
        ...
        while (true) {
            // randomly select two ESTs
            // check genes using EST::getInfo(), if diff. genes break
...
bool
SimulationD2::getAlignmentData(int &alignmentData) {
