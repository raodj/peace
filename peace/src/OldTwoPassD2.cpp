...
int OldTwoPassD2::frameShift = 50;
...
void
OldTwoPassD2::showArguments(std::ostream& os) {
    FWAnalyzer::showArguments(os);
    // Use a arg parser object to conveniently display common options.
    arg_parser ap(OldTwoPassD2::argsList);
    os << ap;
}

bool
OldTwoPassD2::parseArguments(int& argc, char **argv) {
    arg_parser ap(OldTwoPassD2::argsList);
...
int
OldTwoPassD2::setReferenceEST(const int estIdx) {
...
float
OldTwoPassD2::getMetric(const int otherEST) {
    VALIDATE({
...
float
OldTwoPassD2::runD2Asymmetric(const int otherEST, int* s1MinScoreIdx, 
                           int* s2MinScoreIdx) {    
...
float
OldTwoPassD2::runD2Bounded(const int otherEST, int sq1Start, int sq1End, 
