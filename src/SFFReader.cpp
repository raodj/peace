...
SFFReader::getNextRead(const bool maskBases, const bool randomizeNbases) {
    ...
    sffFile.read(&name[0], nameLen);
    VALIDATE_SFF();
    name[nameLen] = '\0'; // Ensure it is proper C-string
    ...
    VALIDATE_SFF();
    // Save offset for EST creation below.
    long offset = (long) sffFile.tellg();
    // Read the actual sequence.
    std::vector<char> sequence;
    sequence.reserve(seqLen + 1);
    ...
    sffFile.seekg(seqLen + dataPad, std::ifstream::cur);
    VALIDATE_SFF();
    ...
    // Now only retain characters between leftIndex and rightIndex
