...
Filter::Filter(const std::string& name, ClusterMaker* clsMaker)
    : filterName(name), clusterMaker(clsMaker) {
    runCount    = 0;
    filterCount = 0;
...
int
Filter::applyFilter(const int estIndex) {
    int clusterIndex;
    if ((clusterIndex = runFilter(estIndex)) != -1) {
        // This EST was filtered out.
        filterCount++;
        // Track the filtered EST information for distribution to
        // other processes later on.
        filteredESTList[clusterIndex].push_back(estIndex);
        // Add this EST to the clsuter
        clusterMaker->addEST(clusterIndex, estIndex);
    }
...
Filter::processFilterData(const std::vector<int>& superList,
    ...
    while (superList[currIndex] != -1) {
        ...
        // Add all ESTs to the dummy cluster.
