...
SepReverseCluster::SepReverseCluster(const std::string& estF, const std::string& mstF) {
    estFileName = estF;
    mstFileName = mstF;
    treeThreshold = 1;
}

void SepReverseCluster::operateC() {
	...
	ProcessEachCluster();
	//cout << "end ProcessEachCluster" << endl;
}
 ...
 */
void SepReverseCluster::readEstFile(const string& inFileName) {
	...
	int i=0;
	//cout << "number of ests: " << ests.size() << endl;
	while (i<ests.size()) {
 ...
 */
void SepReverseCluster::readMST(const string& inFileName) {
...
void SepReverseCluster::ProcessEachCluster() {
	...
	while (true) {
		...
		char curStrIdx [10];
		sprintf(curStrIdx, "%d", estFileIdx++);
		string fName = estFileName + "." + curStrIdx;
		string aceFName = fName + ".aceinfo";
		aceOutFile.open(aceFName.c_str(), ios::trunc);

		while (true) { //put all the nodes in the same MST with 'first' into nodesInCluster
		...
		if (nodesInCluster.size() > 0) {
			writeACluster(nodesInCluster, fName);
			numOfClusters += 1;
		} else {
			estFileIdx--;
		}
	}
	cout << numOfClusters << endl;
}

//write those ESTs in one cluster into a file named "newEstFile".
void SepReverseCluster::writeACluster(vector<int>& nodesInCluster, const std::string& newEstFile) {
	ofstream outFile;
...
vector<stack<int> > SepReverseCluster::getNodesFromMST(vector<stack<int> > nodes) {
...
vector<string> SepReverseCluster::split(const string& str, char delimit) {
...
string SepReverseCluster::toUpperCase(const string& str) {
	string retStr;
	for (int i=0; i<str.size(); i++) {
