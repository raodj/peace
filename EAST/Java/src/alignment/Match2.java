...
//This class is modified by Jenna on 09/10/03 to make it useful for computing A-Score.
...
class ForGene extends Substitution {
	...
	private int[][] residuescores = { /* A G C T N */
			/* A */{ 1 },
			/* G */{ -1, 1 },
			/* C */{ -1, -1, 1 },
			/* T */{ -1, -1, -1, 1 },
...
class NWSmart extends AlignSmart {
	...
	public String[] getMatch() {
		...
		if (n > 1 && m > 1) {
			NWSmart al1, al2;
			al1 = new NWSmart(sub, d, seq1.substring(0, u), seq2
					.substring(0, v));
